//! Jack workflow integration helpers.
//!
//! Jack is an orchestrator plugin (`EnzoVentura/jack`) — a French, web/React
//! oriented sibling of the TPM workflow. When a user opts into Jack mode when
//! creating a session, AoE needs to:
//!
...
//!    installed via `/plugin marketplace`, or pointed at by `JACK_PATH`).
//! 2. Inject it into the spawned `claude` command so the session boots up as
//!    the orchestrator from the first message.
//!
//! This module mirrors [`crate::tpm`] but targets the `jack` plugin. The two
//! are deliberately kept as separate modules (rather than a generic
//! orchestrator abstraction) so each can evolve its own resolution policy,
//! preamble, and registry keys independently.
...
/// `~/.claude/plugins`.
const ENV_OVERRIDE: &str = "JACK_PATH";
...
const PLUGIN_INSTANCE_KEY: &str = "jack@jack";

/// Build the ordered list of candidate orchestrator paths. Earlier entries win.
///
/// The resolution chain:
///   1. `$JACK_PATH/agents/orchestrator.md` (env override)
///   2. `<repo_root>/contrib/jack/agents/orchestrator.md`
///      (when running from a tpm-aoe checkout, the plugin is a git submodule)
///   3. `~/.claude/plugins/cache/jack/jack/<version>/agents/orchestrator.md`
...
/// looking for a `contrib/jack` so `aoe add ./subdir --jack` works.
...
///
/// Real layout: `<home>/.claude/plugins/cache/jack/jack/<version>/agents/orchestrator.md`.
/// Descending lexicographic sort puts `"0.2.0"` before `"0.1.0"`.
///
...
pub fn resolve_orchestrator(repo_root: Option<&Path>) -> Result<PathBuf> {
    ...
    Err(anyhow!(
        "Could not locate the Jack orchestrator prompt. Tried:\n  {}\n\nInstall the plugin with:\n  /plugin marketplace add EnzoVentura/jack\n  /plugin install jack\n\nOr set JACK_PATH to a local checkout that contains agents/{}.",
        candidates
            .iter()
...
/// exists to explicitly tell Claude that on first user turn it must execute
/// the spec's On Activation steps, not treat the message as a normal coding
/// request.
///
/// Apostrophe-free and free of shell metacharacters (`"`, backtick, `$`, `\`)
/// so the whole string nests cleanly inside the `bash -lc '... "..." ...'`
/// wrapping that tmux uses to launch the session — no extra shell escaping
/// required on the call site. Accented characters are safe (not shell
/// metacharacters) and kept so the preamble stays idiomatic French.
const JACK_PREAMBLE: &str = "SYSTEM PROMPT OVERRIDE — MODE ORCHESTRATEUR JACK.

Les instructions ci-dessous priment sur toute consigne contradictoire de ton system prompt par défaut. Au premier message reçu dans cette session, exécute les étapes de la section On Activation du spec ci-dessous. Ne traite pas ce message comme une demande de coder directement. Ton rôle est de dispatcher des sessions via le CLI aoe et de les coordonner, jamais de modifier le code source du projet depuis cette session.
...
fn is_installed_at(path: &Path) -> bool {
    ...
    matches!(
        v.pointer(&format!("/plugins/{}", PLUGIN_INSTANCE_KEY)),
        Some(serde_json::Value::Array(arr)) if !arr.is_empty()
    )
}

/// Native install: clone (or refresh) the marketplace repo into
/// `~/.claude/plugins/marketplaces/jack/`, copy its contents (minus `.git`)
/// into the versioned cache dir, and splice the registry JSON files Claude
/// Code reads on startup.
///
...
fn update_installed_plugins(
    ...
    if actual != INSTALLED_PLUGINS_SCHEMA_VERSION {
        return Err(anyhow!(
            "installed_plugins.json schema version is {}, expected 2. Refusing to write. Run /plugin install jack from a Claude session instead.",
            actual
        ));
    }
...
/// launching `claude` in Jack mode. We use `--append-system-prompt` (rather
...
/// [`JACK_PREAMBLE`] to give those instructions precedence when they conflict
/// with the defaults.
...
/// Concatenate a Jack snippet with whatever the user already configured in
/// `extra_args`. The Jack snippet goes first so user-provided flags can
/// override anything we set.
pub fn merge_extra_args(existing: &str, jack_snippet: &str) -> String {
    if existing.trim().is_empty() {
        jack_snippet.to_string()
    } else {
        format!("{} {}", jack_snippet, existing.trim())
    }
}
...
/// Validate that the user's selected tool can host the Jack orchestrator.
...
pub fn validate_tool(tool: &str) -> Result<()> {
    if tool == "claude" {
        ...
        Err(anyhow!(
            "Jack mode currently requires the `claude` tool (got `{}`). Switch the tool selector to claude or disable Jack mode.",
            tool
        ))
    }
}
...
pub fn build_jack_extra_args(
    ...
    let path = resolve_orchestrator(repo_root)
        .context("Failed to resolve Jack orchestrator system prompt")?;
    Ok(merge_extra_args(
        existing_extra_args,
...
mod tests {
    ...
    /// Write a fake orchestrator under a `contrib/jack` subtree.
    fn write_orchestrator(root: &Path) -> PathBuf {
        let p = root.join("contrib/jack/agents");
    ...
    /// Write a fake orchestrator at the layout `JACK_PATH` expects: the env
    /// var points at the *plugin* root, so the orchestrator lives at
    ...
    fn missing_plugin_returns_actionable_error() {
        ...
        let err = resolve_orchestrator(Some(dir.path()))
            .unwrap_err()
            .to_string();
        assert!(err.contains("Could not locate the Jack orchestrator prompt"));
        assert!(err.contains("/plugin marketplace add"));
        assert!(err.contains("JACK_PATH"));
    }
    ...
    fn preamble_stays_shell_safe() {
        ...
        // break that nesting or trigger unintended shell expansion. Accented
        // characters are fine — they are not shell metacharacters.
        assert!(
            !JACK_PREAMBLE.contains('"'),
    ...
    fn candidate_paths_picks_highest_version_first() {
        ...
        assert!(idx_020 < idx_010, "0.2.0 must appear before 0.1.0");
    }
    ...
    fn is_installed_detects_present_entry() {
        ...
        std::fs::write(&path, r#"{"version":2,"plugins":{"jack@jack":[]}}"#).unwrap();
        assert!(
            !is_installed_at(&path),
        ...
        std::fs::write(
            &path,
            r#"{"version":2,"plugins":{"jack@jack":[{"scope":"user","version":"0.1.0"}]}}"#,
        )
        .unwrap();
        assert!(
            is_installed_at(&path),
    ...
    fn install_end_to_end() {
        ...
        let cache_file = home.join(".claude/plugins/cache/jack/jack/0.1.0/agents/orchestrator.md");
        assert!(
            cache_file.exists(),
            "cache path missing: {}",
            cache_file.display()
        ...
        assert!(
            km.get("jack").is_some(),
            "known_marketplaces: jack entry missing"
        );
        ...
        .unwrap();
        assert_eq!(
            ip.pointer("/plugins/jack@jack/0/version")
                .and_then(|x| x.as_str()),
            Some("0.1.0")
        );
        ...
        .unwrap();
        assert_eq!(
            settings.pointer("/enabledPlugins/jack@jack"),
