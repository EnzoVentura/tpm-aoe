//! Confirmation dialog for installing the jack Claude Code plugin.
...
pub struct JackPluginInstallDialog {
    selected: bool, // true = Accept, false = Cancel
    scroll_offset: u16,
}

impl Default for JackPluginInstallDialog {
    fn default() -> Self {
        Self::new()
    }
}

impl JackPluginInstallDialog {
    pub fn new() -> Self {
        Self {
            selected: true,
            scroll_offset: 0,
    ...
    fn build_content_lines(&self) -> Vec<Line<'_>> {
        vec![
            ...
            Line::from("  ~/.claude/plugins/marketplaces/jack/"),
            Line::from("  ~/.claude/plugins/cache/jack/jack/<version>/"),
            ...
            Line::from("  github.com/EnzoVentura/jack (cloned via git)"),
            Line::from(""),
            Line::from(Span::styled(
    ...
    pub fn render(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let header = Paragraph::new(
            "AoE wants to install the jack plugin into your ~/.claude\ndirectory. Accept to install; cancel to leave Jack mode off.",
        )
...
mod tests {
    ...
    fn default_selection_is_accept() {
        let dialog = JackPluginInstallDialog::new();
        assert!(dialog.selected);
    }

    #[test]
    fn y_submits_true() {
        let mut dialog = JackPluginInstallDialog::new();
    ...
    fn content_lines_mention_key_files() {
        ...
        assert!(text.contains("cache/jack"));
        assert!(text.contains("installed_plugins.json"));
        assert!(text.contains("github.com/EnzoVentura/jack"));
    }
}
